...
### Service accounts that can invoke this service
resource "google_cloud_run_service_iam_member" "sa_invoker" {
  count                   = length(var.invoker_service_account_emails)
  project = google_cloud_run_service.service.project
  location = google_cloud_run_service.service.location
  service = google_cloud_run_service.service.name
  role = "roles/run.invoker"
  member = "serviceAccount:${var.invoker_service_account_emails[count.index]}"
}